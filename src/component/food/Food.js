...
const Food = () => {
  ...
  const getfood = () => {
    ...
    axios.get(url.SERVER_URL + "/api/v2/food/getallitem").then((res) => {
  ...
  const foodcategorys = () => {
    axios.get(url.SERVER_URL + "/api/v2/food/getfoodcategory").then((res) => {
  ...
  const searchproduct = (key) => {
    console.log(key)
    axios.get(url.SERVER_URL + '/api/v2/food/searchproduct/' + key, {
  ...
  const getProducts = (curElem) => {
    ...
    axios.get(url.SERVER_URL + "/api/v2/food/get-categorybasedfooditem", {
  ...
  const getIndivData = async (data) => {
    ...
    await axios.get(url.SERVER_URL + `/api/v2/food/getIndifooddetails?food_uuid=${data}`, {
      headers: { "token": token }
    })
      .then((res) => {
        if (res.data.result) {
  ...
  function addtocart(curElem) {
    ...
    // await setcart(cart=>[...cart,curElem])
  ...
  const tableBookBtn = async () => {
    ...
    await axios.post(url.SERVER_URL + '/api/v2/food/book-table', {
    ...
    <div>
      ...
      <header id="header" className="fixed-top d-flex align-items-cente">
        <div className="container-fluid container-xl d-flex align-items-center justify-content-lg-between">
          ...
          <p className="book-a-table-btn scrollto d-none d-lg-flex" onClick={() => navigate('/book-table')}>Book a table</p>
          ...
          <form role="form" className="php-email-form" data-aos="fade-up" data-aos-delay="100">
            <div className="row">
              <div className="col-lg-4 col-md-6 form-group">
                <input type="text" name="name" className="form-control" id="name" placeholder="Your Name" data-rule="minlen:4" data-msg="Please enter at least 4 chars" onChange={e => setTableData(e.target)} />
                <div className="validate"></div>
              </div>
              <div className="col-lg-4 col-md-6 form-group mt-3 mt-md-0">
                <input type="email" className="form-control" name="email" id="email" placeholder="Your Email" data-rule="email" data-msg="Please enter a valid email" onChange={e => setTableData(e.target)} />
                <div className="validate"></div>
              </div>
              <div className="col-lg-4 col-md-6 form-group mt-3 mt-md-0">
                <input type="text" className="form-control" name="phone" id="phone" placeholder="Your Phone" data-rule="minlen:4" data-msg="Please enter at least 4 chars" onChange={e => setTableData(e.target)} />
                <div className="validate"></div>
              </div>
              <div className="col-lg-4 col-md-6 form-group mt-3">
                <DatePicker className="form-control" selected={bookingDate} onChange={(date) => setBookingDate(date)} />

                <div className="validate"></div>
              </div>
              <div className="col-lg-4 col-md-6 form-group mt-3">
                <TimePicker className="form-control" onChange={setBookingTime} value={bookingTime} />
                <div className="validate"></div>
              </div>
              <div className="col-lg-4 col-md-6 form-group mt-3">
                <input type="text" className="form-control" name="peopleCount" id="peopleCount" placeholder="# of people" data-rule="minlen:1" data-msg="Please enter at least 1 chars" onChange={e => setTableData(e.target)} />
                <div className="validate"></div>
              </div>
            </div>
            <div className="form-group mt-3">
              <textarea className="form-control" name="message" rows="5" placeholder="Message" onChange={e => setTableData(e.target)} ></textarea>
              <div className="validate"></div>
            </div>
            <div className="mb-3">
              <div className="loading">Loading</div>
            ...
            <div className="text-center"><button type="button" onClick={tableBookBtn}>Book a Table</button></div>
          </form>
        </div>
